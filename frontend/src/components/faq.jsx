  ...
  return (
    <div className="w-full h-full bg-no-repeat bg-black bg-center bg-contain flex flex-wrap justify-center gap-2 pb-12">
      <div className="text-white w-full text-center mb-8">
        <h1 className="text-5xl font-bold mb-4">How to use Rubixx?</h1>
        <p className="text-lg mb-4 text-center px-36 text-gray-400 ">
      ...
      {steps.map((item, index) => (
        <div
          key={index}
          className="flex flex-col items-center justify-around gap-2 rounded-2xl shadow-xl bg-blue-400 bg-opacity-25 p-4 mx-8 mb-8 w-1/4"
        >
          <img
            src={item.image}
            alt={`Step ${index + 1}`}
